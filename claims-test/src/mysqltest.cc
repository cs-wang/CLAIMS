...
#include "/home/imdb/git/CLAIMS/Client/Client.h"
#include "/home/imdb/git/CLAIMS/common/log/logging.h"
...
*/
#if 1
#include "/home/imdb/git/CLAIMS/common/Block/DynamicBlockBuffer.h"

extern int utf8_strlen(const std::string & str);

int covert_claims_result(std::string query, int type, ResultSet* rs, string msg)
{
	DYNAMIC_STRING *ds;
    ds= &ds_res;
	if (!disable_query_log )
	{
	  replace_dynstr_append_mem(ds, query.c_str(), query.length());
	//  dynstr_append_mem(ds, delimiter, delimiter_length);
	  dynstr_append_mem(ds, "\n", 1);
	}

	if(type == 0) //type is result 
	{
		int field_count = rs->column_header_list_.size();
		int row_count = rs->getNumberOftuples();

		for(int i = 0 ; i < field_count; i ++)
		{
			dynstr_append_mem(ds, rs->column_header_list_[i].c_str(), rs->column_header_list_[i].length());
			if(i < field_count -1)
			{
				dynstr_append_mem(ds, "\t", 1);
			}
		...
		while(block=it.nextBlock()) {
			...
			while(tuple=tuple_it->nextTuple()){
				for(unsigned i=0;i<rs->column_header_list_.size();i++){
					int current_value_length=utf8_strlen(rs->schema_->getcolumn(i).operate->toString(rs->schema_->getColumnAddess(i,tuple)));
					dynstr_append_mem(ds, rs->schema_->getcolumn(i).operate->toString(rs->schema_->getColumnAddess(i,tuple)).c_str(), current_value_length);
					if(i < field_count -1)
					{
						dynstr_append_mem(ds, "\t", 1);
					}
	...
	else if(type == 1 || type == 2) // message or error
	{
		if (!disable_query_log )
			dynstr_append_mem(ds, msg.c_str(), msg.length());
		//dynstr_append_mem(ds, "\n", 1);
	}
	else
	{
		printf("%s:%s:%d-Error Type: %d",__FILE__, __FUNCTION__, __LINE__, type);
	}
	...
	//MYSQL *mysql= &cn->mysql;
	int err= 0, counter= 0;
// covert claims ResultSet to mysql style res.
{
	extern unsigned int key_memory_MYSQL_RES;
	if (!(res=(MYSQL_RES*) my_malloc(key_memory_MYSQL_RES,
										(uint) (sizeof(MYSQL_RES)+
							sizeof(ulong) *
							rs.column_header_list_.size()),
						MYF(MY_WME | MY_ZEROFILL))))
	{
	...
	res->row_count = rs.getNumberOftuples();
	
	MYSQL_FIELD *pfields =  0;
	MYSQL_DATA	*pdata =  0;
	res->fields = pfields;
	res->data = pdata;
	res->handle = 0;
	extern MYSQL_METHODS client_methods;
	res->methods = &client_methods;
	res->row = 0;
	res->current_row = 0;
	init_alloc_root(0,&res->field_alloc, 8192, 0);
	res->field_count = rs.column_header_list_.size();
	res->current_field = 0;
	res->eof = 1;
	res->unbuffered_fetch_cancelled = 0;
	res->extension = 0;

	{
		pfields= (MYSQL_FIELD*) alloc_root(&(res->field_alloc),
			   (uint) (sizeof(MYSQL_FIELD)*(res->field_count)));

		for(int f = 0; f < res->field_count; f ++)
		{
			string colname = rs.column_header_list_[f];
		//	string org_colname = rs.column_header_list_[f];
			pfields[f].name = (char*)colname.c_str();
			pfields[f].org_name = (char*)colname.c_str();
			pfields[f].table = "t1"; // useless here.
			pfields[f].org_table = "t1"; // useless here.
			pfields[f].db = "dbase"; // useless here.
			pfields[f].catalog = "def"; //useless here.
			pfields[f].def = 0;
			pfields[f].length = 4;// change here
			pfields[f].max_length = 4;//change here
			pfields[f].name_length = strlen(pfields[f].name);
			pfields[f].org_name_length = strlen(pfields[f].org_name);
			pfields[f].table_length = 4;
			pfields[f].org_table_length = 4;
			pfields[f].db_length = 4;
			pfields[f].catalog_length = 3;
			pfields[f].def_length = 0; 
			pfields[f].flags = 32801; 
			pfields[f].decimals = 0;
			pfields[f].charsetnr = 63;
			pfields[f].type = MYSQL_TYPE_SHORT;
			pfields[f].extension = 0x0;
			...
			res->fields = pfields;
		{
			extern unsigned int key_memory_MYSQL_DATA;
			pdata=(MYSQL_DATA*) my_malloc(key_memory_MYSQL_DATA,
                                       sizeof(MYSQL_DATA),
				       MYF(MY_WME | MY_ZEROFILL));
			{
				DBUG_RETURN(0);
			...
			init_alloc_root(PSI_NOT_INSTRUMENTED,
                  &pdata->alloc, 8192, 0); /* Assume rowlength < 8192 */
			pdata->alloc.min_malloc=sizeof(MYSQL_ROWS);
			pdata->embedded_info = 0;
			pdata->rows = res->row_count;
			pdata->fields = res->field_count;
			pdata->extension = 0;

		    MYSQL_ROWS* rows= (MYSQL_ROWS *)alloc_root(&pdata->alloc,
                                  (size_t) ((pdata->rows) * sizeof(MYSQL_ROWS)));

			pdata->data = rows;

			MYSQL_ROWS *prev_row = rows;
			MYSQL_ROWS *end_row = rows + pdata->rows;
			for( ; prev_row < end_row; prev_row ++)
			{
				prev_row->data = 
			}
...
// this block is pulled out from run_query_normal
...
int main(int argc, char **argv)
{
  ...
  if (!(con->name = my_strdup(PSI_NOT_INSTRUMENTED,
                              "default", MYF(MY_WME))))
    die("Out of memory");
  printf("opt_host: [%s], opt_port: [%d]", opt_host, opt_port);
  ...
  Client claimsclient;
  claimsclient.connection(opt_host, opt_port);
  ...
  while (!read_command(&command) && !abort_flag)
  {
    ...
    */
	...
	//run_query(cur_con, command, flags);
	  ...
	  ResultSet rs;
	  std::string message;
	  std::string qusen = command->query_buf;
	  qusen = qusen + ";";
	//  cout << qusen << endl;
	  int msg_type = -1;
	  #if 1
	  switch (claimsclient.submit(qusen, message, rs)) {
		case Client::result:
//		  rs.print();
		msg_type = (int)Client::result;
		  break;
		case Client::message:
		 // printf("%s", message.c_str());
		msg_type = (int)Client::message; 
		  break;
		case Client::error:
		 // printf("%s", message.c_str());
		 msg_type = (int)Client::error;
		  break;
		default:
		  assert(false);
		  break;
	  ...
	  #endif
	  covert_claims_result(qusen, msg_type,  &rs, message);
